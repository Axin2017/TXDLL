...
namespace TXDLL.Tools
{
    ...
    /// 键盘钩子
    /// </summary>
    public class KeyboardHookEvent
    {
        ...
        /// </summary>
        public int WH_KEYBOARD_LL = (int)HookType.GLOBAL_KEYBOAD; 
        HookProc KeyboardHookProcedure; //声明KeyboardHookProcedure作为HookProc类型

        public KeyboardHookEvent()
        {
            if (!string.IsNullOrEmpty(AppConfigTools.GetAppSettingString("HookType")))
            {
                try
                {
                    int h = int.Parse(AppConfigTools.GetAppSettingString("HookType"));
                    if (h==2 || h == 7 || h == 13 || h == 14)
                    {
                        WH_KEYBOARD_LL = h;
                    }
                }
